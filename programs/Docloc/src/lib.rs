...
pub mod docloc {
    ...
    pub fn initialize_escrow(
        ...
        let (pda, _bump_seed) = Pubkey::find_program_address(&[b"docloc"], ctx.program_id);
        token::set_authority(ctx.accounts.into(), AuthorityType::AccountOwner, Some(pda))?;
        Ok(())
    }

    pub fn cancel_escrow(ctx: Context<CancelEscrow>) -> ProgramResult {
        let (_pda, bump_seed) = Pubkey::find_program_address(&[b"docloc"], ctx.program_id);
        let seeds = &[&b"docloc"[..], &[bump_seed]];
